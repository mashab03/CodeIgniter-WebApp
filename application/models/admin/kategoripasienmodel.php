...
class Kategoripasienmodel extends CI_Model {
	...
	function getKategori()
	{
		...
		$url=base_url('admin/kategoripasien').'?paging=true&';		
		$page = isset($_GET['page']) ? mysql_real_escape_string($_GET['page']) : '1';
		
		$limit=20;
		$offset = ($page - 1) * $limit;
		$sql="SELECT * FROM kategori_pasien limit $offset,$limit";
		$sql2="SELECT * FROM kategori_pasien";
	...
	function get_kategori_by_id($id)
	{
		$this->load->library('database_library');
		$this->database_library->pake_table('kategori_pasien');
		$isdata=$this->database_library->ambil_satu_data('id_kategori_pasien',$id,'nama_kategori');
		if(!empty($isdata))
		{
			return $isdata;
		}else{
