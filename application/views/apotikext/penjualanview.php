...
jQuery(document).ready(function(){
	...
	$('input[name^=namaobat]').autocomplete({		
		...
		{	
			...
			this.form.tersedia.value = ui.item.stok_pelengkap;
...
function getcart() {
    $.get('<?= base_url('apotikext/penjualan/lihat');?>', function(data) {
      ...
      $(function () {
		...
		if(Number(qty) > Number(tersedia) || qty=="0")
		{
          ...
          $.ajax({
            type: 'get',
            url: '<?= base_url('apotikext/penjualan/add');?>',
            data: $('form').serialize(),
...
echo form_open(base_url('apotikext/penjualan/checkout'),$att);?>
...
<td><input type="hidden" readonly="readonly" data-validation="number" data-validation-allowing="float" id="harga_beli" class="input-small" name="hargabeli" value="" /><input type="text"  data-validation="number" data-validation-allowing="float" id="harga_jual" class="input-small" name="hargajual" value="" /></td>
